...
const MenuItemComponent = ({ item }: Props) => {
  ...
  const handleAddToCartButtonClick = async (item: MenuItemType) => {
    const cartItem: Partial<CartProductType> = {
    ...
    if (showPopup) {
      addToCart(cartItem, selectedPrice, selectedSize, selectExtras);
      toast.success("Added to cart");
      // await new Promise((resolve) => setTimeout(resolve, 1000));
      setShowPopup(false);
    } else {
      const hasNoOptions = sizes.length === 0 && extraIngredients.length === 0;
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center">
          ...
          >
            ...
            <div className="w-full flex flex-col gap-2 sticky bottom-0">
              ...
              <Button
                className="rounded-lg border text-gray-600 border-transparent hover:border-primary bg-inherit hover:text-primary hover:bg-inherit"
                type="button"
      ...
      <div className="h-full group">
        <div className="bg-gray-200 p-4 rounded-lg text-center hover:bg-white hover:shadow-lg hover:shadow-gray-500 transition-all h-full">
          <div className="h-40">
            <Image
              src={image || "/pizza.png"}
              width={200}
          ...
          <h4 className="font-semibold text-xl my-3 dark:text-gray-800 group-hover:text-primary ">
